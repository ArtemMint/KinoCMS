...
class ChildrenRoomImage(models.Model):
    children_room = models.ForeignKey(
        Page,
        on_delete=models.ForeignKey
    )
    image = models.FileField(
        upload_to='images/children_room'
    )


class CafeBarImage(models.Model):
    cafe_bar = models.ForeignKey(
        Page,
        on_delete=models.ForeignKey
    )
    image = models.FileField(
        upload_to='images/cafe_bar'
    )


class VipHallImage(models.Model):
    vip_hall = models.ForeignKey(
        Page,
        on_delete=models.ForeignKey
    )
    image = models.FileField(
        upload_to='images/vip_hall'
    )
